...
class HybridDBService {
  ...
  async syncToCloud(): Promise<void> {
    ...
    try {
      ...
      // Sync to Firebase
      await Promise.all([
        HybridDatabase.syncPedidosToFirebase(pedidos),
        HybridDatabase.syncSaboresToFirebase(sabores),
        HybridDatabase.syncRellenosToFirebase(rellenos),
        HybridDatabase.syncSettingsToFirebase(settings)
      ]);
  ...
  async getFirebaseUserId(): Promise<string | null> {
    return HybridDatabase.getUserId ? await HybridDatabase.getUserId() : null;
  }
